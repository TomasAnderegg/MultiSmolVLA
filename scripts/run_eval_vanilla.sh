...
# Sanity check: run our 4M pipeline with NO fine-tuned checkpoint (base SmolVLA weights only).
# If this also fails, the issue is in the pipeline architecture/eval setup.
# If it partially succeeds, Stage 2 fine-tuning degraded the policy.
...
OUTPUT_DIR="/home/garate/MultiSmolVLA/eval_results/vanilla_libero_10"
mkdir -p "$OUTPUT_DIR"

# No --checkpoint: uses base SmolVLA weights (lerobot/smolvla_libero) with untrained MLP
